...
scene.clearColor = new BABYLON.Color3(0, 0, 0); // Black
// Create a camera to control it
const camera = new BABYLON.UniversalCamera("camera", new BABYLON.Vector3(0, 6.5, 9), scene);
camera.setTarget(BABYLON.Vector3.Zero());
camera.fov = Math.PI / 2;
camera.aspectRatio = window.innerWidth / window.innerHeight;
camera.minZ = 0.1;
camera.maxZ = 1000;
// Attach the camera controls to the canvas (enables rotation, zoom and panning)
camera.attachControl(canvas, true);
camera.panningSensibility = 50; // How fast the camera pans*
camera.wheelPrecision = 50; // Zoom sensivity
...
pipeline.motionBlurStrength = 0.5; // Ajust motion blur intensity
...
const field = new BABYLON.MeshBuilder.CreateBox("field", {
...
const createCapsuleOutline = (radius, height, radialSegments, scene, color) => {
    const positions = [];
    const indices = [];
    ...
    for (let theta = 0; theta <= Math.PI; theta += segmentStep) {
        ...
        positions.push(new BABYLON.Vector3(x, y, 0));
        if (index > 0) indices.push(index  - 1, index);
        index++;
    }
    // Bottom Half
    for (let theta = 0; theta <= Math.PI; theta += segmentStep) {
    ...
    // Vertical lines
    const topRight = new BABYLON.Vector3(radius, height / 2, 0);
    const botRight = new BABYLON.Vector3(radius, -height / 2, 0);
    const topLeft = new BABYLON.Vector3(-radius, height / 2, 0);
    const botLeft = new BABYLON.Vector3(-radius, -height / 2, 0);

    positions.push(topRight, botRight, topLeft, botLeft);
    indices.push(index, index + 1);
    indices.push(index + 2, index + 3);

    const lines = new BABYLON.MeshBuilder.CreateLineSystem("capsule", {
        lines: [
            positions.slice(0, radialSegments + 1),
            positions.slice(radialSegments + 1, 2 * radialSegments + 1),
            [topRight, botRight],
            [topLeft, botLeft]
        ]
    }, scene);
...
function highlightGoalEffect(position, color, scene) {
    ...
    particleSystem.color1 = new BABYLON.Color3(0, 1, 0);
    particleSystem.color2 = new BABYLON.Color3(0.5, 0, 0.5);
    ...
    particleSystem.createSphereEmitter(1); // Radius of 1
...
function zoomOutEffect(callback) {
    ...
    function zoomOutAnimation() {
        if (camera.position.z < targetPositionZ) {
            ...
            scene.onBeforeRenderObservable.remove(zoomOutAnimation); // Stop animation
            isZooming = false;

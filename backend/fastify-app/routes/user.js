...
async function userRoutes(server) {
    server.route({
        ...
        handler: async (request, reply) => {
            ...
            const result = await Promise.all(users.map(async (tmp) => {
                const user = await (0, user_2.getUserFromDb)({ id: tmp.id });
    ...
    server.route({
        ...
        handler: async (request, reply) => {
            ...
            const user = await (0, user_2.getUserFromDb)({ token });
            if (user == null) {
    ...
    server.route({
        ...
        handler: async (request, reply) => {
            ...
            let user = await (0, user_2.getUserFromDb)({ token });
            if (user == null) {
                reply.code(404).send({ error: "User not found" });
                return;
            }
            ...
            if (password) {
                user.password = js_sha256_1.sha256.hmac(__1.salt, password);
            }
            if (is_online) {
    ...
    server.route({
        ...
        handler: async (request, reply) => {
            ...
            const friend = await (0, user_2.getUserFromDb)({ username: friend_username });
            if (friend == null) {
